...
docker pull docker.artifactory.apps.ecicd.dso.ncps.us-cert.gov/ais-client:latest
...
docker run \
  ...
  -p 443:443 \
  -p 8083:8083 \
   --add-host ais2-pfte.taxii.us-cert.gov:10.14.0.11 \
   --name client \
   docker.artifactory.apps.ecicd.dso.ncps.us-cert.gov/ais-client:latest &
