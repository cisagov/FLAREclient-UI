...
BASE_REBUILD=false
FLARECLIENT_UI_REBUILD=true
FLARECLIENT_JAVA_REBUILD=false
...
if [[ "$(docker images -q client-base:latest 2> /dev/null)" == "" || ${BASE_REBUILD} ]]; then
...
if ${FLARECLIENT_JAVA_REBUILD}; then
...
if ${FLARECLIENT_UI_REBUILD}; then
  npm install
