...
import { clearStatus, getStatus } from '../../redux/actions/status';
...
function Statuses({className, staticContext, ...rest}) {
  ...
  const [clearedFlag, setClearedFlag] = useState(null);
  ...
  const execClearStatus = async (statusId) => {
    try {
      console.log('[ ] Clear status(initial) ' + statusId);
      await dispatch(clearStatus(statusId));
      console.log('[*] Clear status(initial) ' + statusId + ' finished!');
      fetchStatus();
    } catch (err) {
      console.log('[x] Error clearing status', err);
    }
  };
                      ...
                      <TableCell id={"warnings"}>
                        {status.errorCount > 0 ?
                          (status.errorCount >= 10 ?
                            (<div>
                              <Tooltip title={"Error: Error getting status " + status.errorCount + " times."}>
                                <CancelIcon />
                              </Tooltip>
                              <br/>
                              <Tooltip title={"Click 'retry' to reattempt status updates."}>
                                <Link style={{color: '#FFF'}} onClick={() => {execClearStatus(status.id)}}>retry</Link>
                              </Tooltip>
                            </div>) :
